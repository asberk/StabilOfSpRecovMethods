...
display(sprintf(['Given b(j,k) = (A*x0)(j) + epsilon(k)*z(j,k),\n',...
...
[lasso_error, qp_lambda] = batchCvxError(A,b,x0, 'lasso', tau_vec, opts, verbose);
...
for ni = 1:nIter^2
    ...
    surf(X,Y,reshape(qp_lambda(:, ni, :), size(tau_vec,2), 25));
    title(ni);
    pause;
    plot(tau_vec, reshape(qp_lambda(:,ni,:), size(tau_vec,2),25));
    title(ni);
    pause;
end

%% Use qp_lambda to obtain tau-lambda plot
firstMins = zeros(nIter^2, 25); 
figure;
hold on;
for ni = 1:nIter^2
    for lam = 1:25
        firstMins(ni, lam) = tau_vec(findFirstMin(qp_lambda(:, ni, lam)));
